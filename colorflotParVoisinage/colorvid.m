                ...
                if im > 1
                    ...
                    for vlig=max(1,antY-1):min(hauteur,antY+1) %pour ligne avec prise en compte bord
                        for vcol=max(1,antX-1):min(largeur,antX+1)%pour colonne avec prise en compte bord
                           ...
                           lenvaleurs=lenvaleurs+1;
                           lignes(lenvaleurs)=largeur*hauteur*(im-1)+largeur*(lig-1)+col; %ligne de la valeur c'est coordoonnées du pixel etudie
                           colonnes(lenvaleurs)=largeur*hauteur*(im-2)+largeur*(vlig-1)+vcol; %colonne de la valeur c'est coodoonnées voisin étudié
                           voisins(lenvois)=nbvid(vlig,vcol,1,im-1); %dans vecteur voisin on met intensité du voisin
                        end 
                    end
                end
                ...
                if im < profondeur
                    ...
                    for vlig=max(1,postY-1):min(hauteur,postY+1) %pour ligne avec prise enc ompte bord
                        for vcol=max(1,postX-1):min(largeur,postX+1)%pour colonne avec prise en compte bord
                           ...
                           lenvaleurs=lenvaleurs+1;
                           lignes(lenvaleurs)=largeur*hauteur*(im-1)+largeur*(lig-1)+col; %ligne de la valeur c'est coordoonnées du pixel etudie
                           colonnes(lenvaleurs)=largeur*hauteur*(im)+largeur*(vlig-1)+vcol; %colonne de la valeur c'est coodoonnées voisin étudié
                           voisins(lenvois)=nbvid(vlig,vcol,1,im+1); %dans vecteur voisin on met intensité du voisin
                        end 
                    end
                end
